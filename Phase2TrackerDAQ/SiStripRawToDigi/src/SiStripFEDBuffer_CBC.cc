...
#include "Phase2TrackerDAQ/SiStripRawToDigi/interface/utils.h"
...
namespace sistrip
{
  ...
  // implementation of FEDBuffer_CBC
  FEDBuffer_CBC::FEDBuffer_CBC(const uint8_t* fedBuffer, const size_t fedBufferSize) 
    : buffer_(fedBuffer),
      bufferSize_(fedBufferSize)
  {
      LogTrace("FEDBuffer_CBC") << "content of buffer with size: "<<int(fedBufferSize)<<std::endl;
      for ( size_t i = 0;  i < fedBufferSize; i += 8)
      {
        uint64_t word  = read64(i,buffer_);
        LogTrace("FEDBuffer_CBC") << " word " << std::setfill(' ') << std::setw(2) << i/8 << " | " 
        << std::hex << std::setw(16) << std::setfill('0') << word << std::dec << std::endl;
      }
      LogTrace("FEDBuffer_CBC") << std::endl;
      ...
      trackerHeader_ = TrackerHeader_CBC(buffer_+8);
  ...
  FEDBuffer_CBC::~FEDBuffer_CBC()
  {
  }
  
  void FEDBuffer_CBC::findChannels()
  {
    ...
    if(readoutMode() == READOUT_MODE_PROC_RAW)
    {
      for (FE_it = status.begin(); FE_it < status.end(); FE_it++)
      {
        ...
        if(*FE_it)
        {
          ...
          for (int i=0; i<MAX_CBC_PER_FE; i++) 
          {
            ...
            if((cbc_status>>i)&0x1)
            {
              ...
              channels_.push_back(FEDChannel_CBC(payloadPointer_,offsetBeginningOfChannel,(STRIPS_PADDING+STRIPS_PER_CBC)/8));
              offsetBeginningOfChannel += (STRIPS_PADDING+STRIPS_PER_CBC)/8;
            }
            else
            {
              channels_.push_back(FEDChannel_CBC(NULL,0,0));
            }
          }
        }
        else
        {
          ...
          channels_.insert(channels_.end(),size_t(MAX_CBC_PER_FE),FEDChannel_CBC(payloadPointer_,0,0));
        }
      }
    }
      ...
      for (FE_it = status.begin(); FE_it < status.end(); FE_it++)
      {
        if(*FE_it)
        {
          for (int i=0; i<MAX_CBC_PER_FE; i++)
          {
            ...
            channels_.push_back(FEDChannel_CBC(payloadPointer_,offsetBeginningOfChannel,2*n_clusters));
            // skip clusters
    ...
    else
    {
      ...
      // check done at TrackerHeader_CBC::readoutMode()
    }
    ...
    if(trackerHeader_.getConditionData())
    {
      ...
      if (bufferDiff <= 0)
      {
        ...
        ss << "[sistrip::FEDBuffer_CBC::"<<__func__<<"] " << "\n";
        ...
        throw cms::Exception("FEDBuffer_CBC") << ss.str();
      }
    }
    else
    {
  ...
  std::map<uint32_t,uint32_t> FEDBuffer_CBC::conditionData()
  {
      ...
      if(condDataPointer_)
      {
        ...
        LogTrace("FEDBuffer_CBC") << "Condition Data size = " << size << std::endl;
        pointer+=8;
        // now the conditions
        while(pointer < stop) 
        {
  ...
  FEDReadoutMode FEDBuffer_CBC::readoutMode() const
  {
...
////////////////////////////////////////////////////////////////////////////////
  ...
  TrackerHeader_CBC::TrackerHeader_CBC(const uint8_t* headerPointer) 
  ...
  void TrackerHeader_CBC::init() 
  {
    ...
    {
      LogTrace("FEDBuffer_CBC")
        << "[sistrip::TrackerHeader_CBC::"<<__func__<<"]: \n"
  ...
  uint8_t TrackerHeader_CBC::dataFormatVersion() const
  {
    uint8_t Version = static_cast<uint8_t>(extract64(VERSION_M, VERSION_S, header_first_word_));
    if (Version != 1)
    {
      std::ostringstream ss;
      ss << "[sistrip::TrackerHeader_CBC::"<<__func__<<"] ";
  ...
  READ_MODE TrackerHeader_CBC::debugMode() const
  {
  ...
  uint8_t TrackerHeader_CBC::eventType() const
  {
  ...
  FEDReadoutMode TrackerHeader_CBC::readoutMode() const
  {
  ...
  uint8_t TrackerHeader_CBC::conditionData() const
  {
    return static_cast<uint8_t> (eventType_ >>1) & 0x1;
  }

  uint8_t TrackerHeader_CBC::dataType() const
  {
    return static_cast<uint8_t> (eventType_) & 0x1;
  }
  
  uint64_t TrackerHeader_CBC::glibStatusCode() const
  {
    return extract64(GLIB_STATUS_M, GLIB_STATUS_S, header_first_word_);
  }
  
  std::vector<bool> TrackerHeader_CBC::frontendStatus() const
  {
  ...
  uint16_t TrackerHeader_CBC::numberOfCBC() const
  {
  ...
  // pending too
  std::vector<uint8_t> TrackerHeader_CBC::CBCStatus() const
  {
  ...
  const uint8_t* TrackerHeader_CBC::pointerToData()
  {
    int status_size = 0;
